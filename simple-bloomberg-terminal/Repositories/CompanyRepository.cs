...
public class CompanyRepository(AppDbContext db) : ICompanyRepository
{
    ...
    public Company? GetWithGraphRelations(long id) =>
        db.Companies
            ...
            .Include(c => c.RevenueSources).ThenInclude(r => r.Filing)
            .Include(c => c.CostSources).ThenInclude(c2 => c2.RelatedCompany)
            .Include(c => c.CostSources).ThenInclude(c2 => c2.Filing)
            .Include(c => c.CompanyRisks).ThenInclude(r => r.Filing)
    ...
    internal static readonly HashSet<string> NameNoise = new(StringComparer.Ordinal)
    {
