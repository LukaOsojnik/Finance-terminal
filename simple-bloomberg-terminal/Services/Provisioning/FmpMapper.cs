...
/// (then rolled up) by <see cref="IIndustryClassifier"/> â€” no hand-maintained label dictionary here.
/// </summary>
public static class FmpMapper
{
    public static CompanyCreateModel ToCreateModel(FmpProfile p, FmpIncome? income)
    {
        ...
        if (income != null)
        {
            // Stable income has no ratio fields, so derive margin = grossProfit / revenue (0â€“1).
            ...
            if (DateOnly.TryParse(income.Date, out var d))
                model.AsOf = d;
            // RevenueTotal is a plain USD double â€” only fill it when FMP reported in USD.
    ...
    // Strip everything but letters/digits and lowercase, so "Softwareâ€”Infrastructure",
