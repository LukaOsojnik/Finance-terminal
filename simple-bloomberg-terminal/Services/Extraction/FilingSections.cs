...
public static class FilingSections
{
    ...
    // Overall safety cap, sized at MaxChunksPerSection × the widest routing (REVENUE's four Items:
    // 1, 1A, 7, 8). It must stay in step with ItemsFor: this cap is checked AFTER the per-section
    // break, so at 3×12 a fourth Item got exactly one chunk before Build returned — and Item 8, the
    // one carrying the actual figures, is last in document order and so was the Item that lost.
    ...
    // Topic words that mark a Details report as relevant to a node, matched against its ShortName.
    // Drawn from the section titles filers actually use ("Segment Information and Geographic Data -
    // Net Sales (Details)"), which are conventional enough across filers to match on.
