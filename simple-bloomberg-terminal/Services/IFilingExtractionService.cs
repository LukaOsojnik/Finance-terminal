...
public interface IFilingExtractionService
{
    ...
    /// <summary>The chat's grounding digest for a filing — cached; built by the all-sections auto-scan
    /// on a miss, or pre-populated by <see cref="ScanSelectedHeadingsAsync"/>.</summary>
    Task<string> GetOrScanDigestAsync(
        long companyId, string accession, string doc, CancellationToken ct = default);

    /// <summary>Bold sub-headings inside Items 7/8/1A for the user to pick from.</summary>
